...
impl App {
        ...
        match event {
            InputEvent::Key(e) => {
                if single_tile_debug_enabled() {
                    let phase = if e.is_down { "KeyDown" } else { "KeyUp" };
                    eprintln!("[overlay-single] {phase} label={}", e.key);
                }
            }
            InputEvent::MouseClick(e) => {
                if e.is_down {
    ...
    fn build_render_tokens(&mut self, now: Instant, dt: f32) -> Vec<RenderToken> {
        ...
        let combo_mode = !modifiers_down.is_empty() && !non_modifiers_down.is_empty();
        ...
        if combo_mode {
            let mod_label = modifiers_down
                ...
                .collect::<Vec<_>>()
                .join(" + ");
            for key in non_modifiers_down {
                tokens.push(RenderToken {
                    label: format!("{} + {}", mod_label, key.to_string().to_uppercase()),
                    is_mouse: false,
                    mouse_highlight: MouseHighlight::None,
                    alpha: 1.0,
                    scale: 1.0,
            ...
            let mut individual_keys: Vec<Key> = pressed.iter().copied().collect();
            individual_keys.sort_by_key(|k| key_sort_rank(*k));
            for key in individual_keys {
                tokens.push(RenderToken {
                    label: key.to_string().to_uppercase(),
                    is_mouse: false,
                    mouse_highlight: MouseHighlight::None,
                    alpha: 1.0,
                    scale: 1.0,
        ...
        if single_tile_debug_enabled() {
            eprintln!(
                "[overlay-single] tokens={} held_mouse={} held_keys={} combos={} ",
                tokens.len(),
                self.tray_state
                    .active_mouse
                    .values()
...
impl eframe::App for App {
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        while let Ok(event) = self.rx.try_recv() {
            ...
            if repaint_debug_enabled() {
                if let InputEvent::Key(k) = &event {
                    if k.is_down && matches!(k.key, Key::Alt) {
                        let overlay_fg = self.last_hwnd.map(is_foreground_window).unwrap_or(false);
                        eprintln!(
                            "[overlay-repaint] ALT down foreground_overlay={} hwnd={:?}",
                            overlay_fg, self.last_hwnd
                        );
                    }
                }
            }
        ...
        if processed_events > 0 {
            if repaint_debug_enabled() {
                eprintln!(
                    "[overlay-repaint] processed_events={} active_until+250ms; request_repaint",
                    processed_events
                );
            }
            ctx.request_repaint();
        ...
        *self.config.lock().unwrap() = self.draft.clone();
        ...
        if keys_held || mouse_held || now < self.active_until {
            if repaint_debug_enabled() {
                eprintln!(
                    "[overlay-repaint] active refresh keys_held={} mouse_held={} until_active={}",
                    keys_held,
                    mouse_held,
...
pub fn run(
    ...
    eframe::run_native(
        ...
        options,
        Box::new(move |cc| {
            register_external_repaint_ctx(&cc.egui_ctx);
            Box::new(App::new(rx, config))
        }),
    )
