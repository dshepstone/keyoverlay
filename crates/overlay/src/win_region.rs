...
mod imp {
    ...
    pub fn log_win_state(hwnd: HWND, label: &str) {
        ...
        startup_debug::log(format!(
            "win_state[{label}] hwnd={hwnd:?} visible={visible} style=0x{style:08X} exstyle=0x{exstyle:08X} \
    ...
    pub fn apply_tray_region_with_redraw(
        ...
        log_win_state(hwnd, "before-setwindowrgn");
        ...
        log_win_state(hwnd, "after-setwindowrgn");
        Some(hwnd)
    }

    pub fn force_redraw(hwnd: HWND) {
        startup_debug::log(format!("force redraw hwnd={hwnd:?}"));
...
pub use imp::{
    ...
    find_hwnd_by_title, force_redraw, log_win_state,
};
...
#[cfg(not(target_os = "windows"))]
pub fn log_win_state(_hwnd: OverlayHwnd, _label: &str) {}
